...
export function config() {
  ...
  // Prefer Anthropic when both are configured; otherwise the only one configured.
...
export async function complete({ system, user, maxTokens = 3000 }) {
  ...
  if (!provider) {
    const err = new Error('No LLM provider configured. Set ANTHROPIC_API_KEY or OPENAI_API_KEY in server/.env');
    err.status = 503;
    throw err;
  }

  if (provider.name === 'anthropic') return completeAnthropic(provider.key, { system, user, maxTokens });
  return completeOpenAI(provider.key, { system, user, maxTokens });
}
